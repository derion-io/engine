...
class Resource {
    ...
    async loadInitPoolsData(listTokens, poolAddresses, playMode) {
        try {
            const mc = new ethereum_multicall_1.Multicall({
            ...
            const chainlinkPools = lodash_1.default.uniq(Object.values(pools)
                .filter((p) => (0, helper_1.isChainlink)(p))
                .map((p) => p.pair));
            const chainlinkFeedsData = {};
            if (chainlinkPools.length) {
                await (0, multicall_1.multicall)(this.provider, lodash_1.default.uniq(chainlinkPools).map(p => {
                    return {
                        reference: p,
                        contractAddress: p,
                        ...
                        context: (callsReturnContext) => {
                            for (const ret of callsReturnContext) {
                                chainlinkFeedsData[p] = { description: ret.returnValues[0] };
                            }
                        }
                    };
                }), true);
            ...
            const uni3Pools = Object.values(pools)
                .filter((p) => (0, helper_1.isUniv3)(p))
                .map((p) => p.pair);
            let pairsInfo = {};
            if (uni3Pools.length) {
                pairsInfo = await this.UNIV3PAIR.getPairsInfo({
                    pairAddresses: lodash_1.default.uniq(uni3Pools),
                });
            }
            const tokens = [];
            ...
            for (const i in pools) {
                ...
                let baseToken;
                let quoteToken;
                if ((0, helper_1.isChainlink)(pools[i]) && chainlinkFeedsData[pools[i].pair]) {
    ...
    async searchIndex(keyword) {
        try {
            ...
            const pairsInfo = {};
            const chainlinkFeeds = Object.values(poolGroups).filter((p) => (0, helper_1.isChainlink)(p.pools[0])).map((pg) => pg.pairAddress);
            const promiseChainlink = !chainlinkFeeds.length ? null : (0, multicall_1.multicall)(this.provider, chainlinkFeeds.map(feed => {
                return {
                    reference: feed,
            ...
            const uniPairs = Object.values(poolGroups).filter((p) => (0, helper_1.isUniv3)(p)).map((pg) => pg.pairAddress);
            if (!uniPairs.length) {
            ...
            poolAddresses.forEach((poolAddress) => {
                try {
                    ...
                    pools[poolAddress] = {
                        ...
                        window: (0, helper_1.bn)((0, helper_1.oracleWindow)(configData.config.ORACLE)),
